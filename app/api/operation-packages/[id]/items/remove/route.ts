...
export async function POST(request: Request, { params }: { params: { id: string } }) {
  try {
    ...
    console.error("Error removing items from operation package:", error)
    return NextResponse.json({ error: "Failed to remove items from operation package" }, { status: 500 })
  }
}
