...
type FilterableColumns = "location" | "year"

export function OperationPackageDataTable<TData, TValue>({
  columns,
  data,
  enableFilters = true,
  ...
  const uniqueValues = useMemo(() => {
    ...
    data.forEach((item: any) => {
      ...
      values.year.add(item.year)
    })

    return {
      location: Array.from(values.location),
      year: Array.from(values.year),
    }
  }, [data])
  ...
  const getActiveFilters = () => {
    return (["location", "year"] as const).flatMap((columnId) => {
      const column = table.getColumn(columnId)
    ...
    <div className="space-y-4">
      <div className="flex items-center gap-2">
        ...
        <div className="flex items-center gap-2">
          {enableFilters && (
            <div className="flex items-center gap-2">
              {(["location", "year"] as const).map((columnId) => (
                <Select
                  key={columnId}
